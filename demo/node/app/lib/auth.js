...
function userByEmail(email) {
  return getDb().prepare("SELECT id, email, name, email_verified, password_hash, created_at FROM users WHERE email = ?").get(email);
}
...
function createToken(identifier) {
  const token = uuid();
  getDb().prepare("INSERT INTO verification_tokens VALUES (?, ?, ?, ?, ?)").run(uuid(), identifier, token, new Date(Date.now() + DAY).toISOString(), now());
  return token;
}

function tokenRow(token) {
  const row = getDb().prepare("SELECT id, identifier, expires_at FROM verification_tokens WHERE token = ?").get(token);
  if (!row) return null;
  if (Date.parse(row.expires_at) < Date.now()) {
    getDb().prepare("DELETE FROM verification_tokens WHERE id = ?").run(row.id);
    return null;
  }
  return row;
}
...
export async function signUp(request) {
  ...
  try {
    const createdAt = now();
    getDb().prepare("INSERT INTO users VALUES (?, ?, ?, ?, 0, ?, ?)").run(uuid(), normalizedEmail, await bcrypt.hash(password, 12), normalizedEmail, createdAt, createdAt);
...
export async function signIn(request) {
  ...
  if (!user || !(await bcrypt.compare(password, user.password_hash))) return error("invalid email or password", 401);
  ...
  const token = uuid();
  getDb().prepare("INSERT INTO sessions VALUES (?, ?, ?, ?, ?)").run(uuid(), user.id, token, new Date(Date.now() + 30 * DAY).toISOString(), now());
...
export function session(request) {
  ...
  const row = getDb().prepare("SELECT user_id, expires_at FROM sessions WHERE token = ?").get(token);
  if (!row || Date.parse(row.expires_at) < Date.now()) {
    if (row) getDb().prepare("DELETE FROM sessions WHERE token = ?").run(token);
    return json({ user: null });
  }
  const user = getDb().prepare("SELECT id, email, name, email_verified, created_at FROM users WHERE id = ?").get(row.user_id);
...
export async function verifyEmail(request) {
  ...
  getDb().prepare("UPDATE users SET email_verified = 1 WHERE email = ?").run(row.identifier);
  getDb().prepare("DELETE FROM verification_tokens WHERE id = ?").run(row.id);
...
export async function resetPassword(request) {
  ...
  getDb().prepare("UPDATE users SET password_hash = ?, updated_at = ? WHERE email = ?").run(await bcrypt.hash(password, 12), now(), row.identifier);
  getDb().prepare("DELETE FROM verification_tokens WHERE id = ?").run(row.id);
  getDb().prepare("DELETE FROM sessions WHERE user_id IN (SELECT id FROM users WHERE email = ?)").run(row.identifier);
